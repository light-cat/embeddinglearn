...
uint8_t BMP_ReadOneByte(uint8_t ReadAddr)
{
	...
	IIC_Send_Byte(0xEE);  //bmp180器件的地址，写入方向为EE,读出方向为EF
	IIC_Wait_Ack();   //等待应答

	IIC_Send_Byte(ReadAddr); // 发送要读取的地址 及 读写命令
	IIC_Wait_Ack(); //等待应答
	
	IIC_Start();  
	
	IIC_Send_Byte(0xEF); // bmp180器件的地址，写入方向为EE,读出方向为EF
	IIC_Wait_Ack();  // 等待应答
	
	data = IIC_Read_Byte(1); // 读取一个字节的数据
	IIC_Stop();
